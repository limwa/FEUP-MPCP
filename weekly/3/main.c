...
extern int OpMat(unsigned char nc, unsigned char nl, unsigned char *o, unsigned char *v);
...
int main(void) {
    ...
    int res = OpMat(ncol, nlin, ops2, mat);
    printf("Retorno de OpMat = %d\n", res);

    for (char i = 0; i < nlin; i++) {

        for (char j = 0; j < ncol; j++)
